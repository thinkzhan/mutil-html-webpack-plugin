...
class MutilHtmlWebpackPlugin {
    ...
    constructor(options) {
        ...
        pageNames.forEach(pageName => {
            let templatePath = path.resolve(page, pageName, `${ext || '.vm'}`);

            let entryPath = path.resolve(page, pageName, 'page.js');;
            let htmlOutputFilename = path.join(toPage || '',
                `${pageName}${toExt || '.vm'}`);
    ...
    apply(compiler) {
        ...
        compiler.plugin('emit', (compilation, callback) => {
            if (pagemap) {
                ...
                util.addFileToWebpackOutput(compilation, 'pagemap.json', JSON.stringify(
                    outJson));
            }
            callback();
        });
